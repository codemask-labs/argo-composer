...
const envTemplate = (envName: string, options: Options) =>
    mergeWith(
        apply(url(`./overlay`), [
            ...
            move(
                `/projects/${options.projectName}/apps/${options.appName}/overlays/${envName}`
            ),
        ])
    );
...
const addResources = (options: Options) =>
    mergeWith(
        apply(url(`./resources`), [
            template({ ...options, ...strings }),
            move(
                `/projects/${options.projectName}/apps/${options.appName}/resources`
            ),
        ])
    );
...
export const add = (): Rule => async (tree: Tree) => {
    ...
    const base =
        overlays.length > 0 && shouldAppContainOverlays
