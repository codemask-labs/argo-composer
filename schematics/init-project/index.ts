import { apply, Tree, url, template, strings, chain, mergeWith, Rule, SchematicsException } from '@angular-devkit/schematics'
...
const updateProjectKustomization =
    (projectName: string, name?: string): Rule =>
    (tree: Tree): Tree => {
        if (!name) {
            return tree
        }
...
const updateAddonsKustomization =
    (projectName: string, addons: Array<string>, addonsProjectName?: string): Rule =>
    (tree: Tree): Tree => {
        ...
        const updatedYaml = {
            resources: [...newApplicationDirectories]
        }
...
export const addonTemplate = (addon: string, options: InitProjectSchematicOptions, addonsProjectName: string) =>
    mergeWith(apply(url(`./addons/${addon}`), [template({ ...options, ...strings, addonsProjectName })]))
