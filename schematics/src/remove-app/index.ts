...
const updateKustomization = (projectName: string, appName: string): Rule => {
  return (tree: Tree): Tree => {
    ...
    const appPath = `./${appName}.yaml`
    const updatedYaml = {
      ...yaml,
      resources: yaml.resources.filter((resource: string) => resource !== appPath)
    }
...
export const remove = (): Rule => {
  return async (tree: Tree) => {
    ...
    const projectName = await select<string>({
      message: 'From which project you want to remove app?',
    ...
    const confirmation = await confirm({ message: `Are you sure want to remove app ${projectName}/apps/${appName}?` })
